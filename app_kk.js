    ...
    function handleSend() {
        ...
        setTimeout(() => {
            ...
            if (lowerText.includes("өлгім") || lowerText.includes("өлу") || lowerText.includes("өлім") || lowerText.includes("суицид") || lowerText.includes("өзімді өлтіру") || lowerText.includes("өмір сүргім келмейді") || lowerText.includes("смерть") || lowerText.includes("умру")) {
                ...
                isCrisis = true;
            } else if (lowerText.includes("көмек") || lowerText.includes("көмектес") || lowerText.includes("помоги") || lowerText.includes("поможешь") || lowerText.includes("қолыңнан") || lowerText.includes("істей аласың")) {
                botResponse = "Мен — «Близкие Друзья» қауымдастығының виртуалды көмекшісімін. Біздің кеңістік пен бағдарлама сізге ішкі тепе-теңдікті қалпына келтіруге, жеке шекараңызды орнатуға және қолдаушы орта табуға қалай көмектесетінін айта аламын. Клубтың өзінде сізге кәсіби психологтар, тәжірибелі кураторлар және бір-бірін қолдайтын әйелдер қауымдастығы көмектеседі. Қазір сізді не мазалайтынын айта аласыз ба?";
